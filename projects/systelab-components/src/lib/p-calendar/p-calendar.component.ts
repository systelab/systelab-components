...
import { OverlayService } from './api/overlayservice';
import { PrimeNGConfig } from './api/primengconfig';
import { PrimeTemplate } from './api/shared';
import { TranslationKeys } from './api/translationkeys';
...
export const CALENDAR_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => PCalendarComponent),
  multi: true
};
...
export class PCalendarComponent implements OnInit, OnDestroy, ControlValueAccessor {
  ...
  @Input() style: any;
    ...
    isDateEquals(value, dateMeta) {
        if (value && this.isDate(value)) return value.getDate() === dateMeta.day && value.getMonth() === dateMeta.month && value.getFullYear() === dateMeta.year;
        else return false;
    }
    ...
    isValidDate(date) {
        return this.isDate(date) && ObjectUtils.isNotEmpty(date);
    }
