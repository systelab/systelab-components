...
| autoResizableScroll | boolean |                 false                 | **Only for AbstractGrid (non-API grids).** If true, automatically preserves the user's scroll position when rows are added, deleted, or modified. When rows are added, scrolls to bottom to show new entries. When rows are deleted, maintains relative position or adjusts if viewing bottom. When rows are edited without count changes, maintains exact scroll position. Prevents disruptive scroll jumps during data updates. |
