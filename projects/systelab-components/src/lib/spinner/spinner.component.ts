...
export class TouchspinComponent {
	...
	@Input() fillUnitsWithZero:boolean|number = false; //0 if false, and 1 if true
	...
	set value(val: number) {
		if (this._spinValues) {
			...
			if (val) {
				const valStr: string = val >0 ? String(val).padStart(numberOfZeros+1, '0'): String(val).substring(0,1)+String(val).substring(1).padStart(numberOfZeros+1, '0');
